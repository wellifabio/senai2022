...
public class Menu {
	...
	private static String[] funcionarios = { "Marcelo             ", "Mariana             " };
	private static String[] descServicos = { "Banho               ", "Tosa                ", "Outro               " };
	...
	private static void cadastrarServicos() throws ParseException {
		boolean sucesso = false;
		int opcao = menu(new String[] { "Cadastrar retroativo", "Cadastrar agora" });
		switch (opcao) {
		case 1:
			sucesso = cadastrarRetroativo();
			if (sucesso) {
	...
	private static boolean cadastrarAgora() throws ParseException {
		System.out.println("Para registrar um serviço, o pet deve estar cadastrado");
		...
		if (animal != null) {
			...
			String funcionario = funcionarios[menu(funcionarios) - 1];
			String servico = descServicos[menu(descServicos) - 1];
			servicos[contServicos] = new Servico(animal, funcionario, servico);
			return true;
		} else {
	...
	// Médodo que cadastra um serviço/objeto no vetor
	private static boolean cadastrarRetroativo() throws ParseException {
		System.out.println("Para registrar um serviço, o pet deve estar cadastrado");
		System.out.println("Digite qual o id do pet/animal?");
		Pet animal = buscarAnimal(read.nextInt());
		if (animal != null) {
	...
	private static void listarServicos() throws ParseException {
		...
		String[] subItens = { "Filtrar Serviços    ", "Filtrar Por data    ", "Não filtrar         " };
		int sub = 0;
		sub = menu(subItens);
		if (sub == 1) {
			...
			System.out.println("Continue navegando no sistema.");
		} else {
