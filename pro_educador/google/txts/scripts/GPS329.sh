# Executar este script em duas etapas
export SANAME=challenge
export PROJECT_ID=qwiklabs-gcp-01-46c0213841f9
export LANG=en
export ROLE_BIGQUERY=roles/bigquery.dataOwner
export ROLE_CLOUDSTORAGE=roles/storage.objectAdmin
echo $SANAME
echo $PROJECT_ID
echo $LANG
echo $ROLE_BIGQUERY
...
# Primeiro até aqui, configurando as variáveis de ambiente, projeto e conta de serviço concluindo a tarefa 1
# Em seguida daqui para baixo concluindo as outras tarefas 2, 3, 4 e 5.
...
for file in files:    
    if file.name.endswith('jpg') or  file.name.endswith('png'):
        ...
        if locale == os.environ['LANG']:
            ...
            translation = translate_client.translate(text_data, target_language=os.environ['LANG'])
...
read -p "POR FAVOR AGUARDE 1min" -t 60
