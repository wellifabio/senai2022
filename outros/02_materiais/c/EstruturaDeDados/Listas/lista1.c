...
bool inserirElemLista(Lista* list, Registro reg, int posicao){//Ponteiro da Lista, Registro com a Posição na Lista
	int i; //Contador para o laço for
	if((list->nroElem == MAX)||(posicao < 0)||(posicao > list->nroElem)) //Testa as condições básicas para INSERIR
	...
	for(i = list->nroElem; i > posicao; i--) list->A[i] = list->A[i-1]; //Desloca toda a Lista uma posição para frente (da posição informada em dian
	list->A[posicao] = reg; //Atribui o Registro lido a posição do contador i (Já fora do laço for)
...
bool excluirElemLista(TipoChave ch, Lista* list){//O índice CHAVE do elemento a ser excluído e o ponteiro da Lista
	...
	for(i = posicao; i < list->nroElem; i++) list->A[i] = list->A[i+1];//Desloca toda a Lista uma posição para traz (Desta forma removendo o elemento)
...
int main(){
	...
	int chave, posicao; //Variáveis para ler a chave e a posicao
	...
	do{
		...
		switch(opcao){
			case 1:
				printf("Digite a posicao na Lista iniciando em 0: "); scanf("%d",&posicao); //Le a posição do elemento na lista
			...
			case 4:
				...
				printf("Digite a chave a ser encontrada na Lista: "); scanf("%d",&chave); //Le a posição do elemento na lista
